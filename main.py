...
try:
    ...
    print("\nvRY Mobile", color(
        f"- {get_ip()}:{cfg.port}", fore=(255, 127, 80)))
    ...
    while True:
        ...
        if True:
            ...
            if game_state == "INGAME":
                ...
                loadouts_arr = loadoutsClass.get_match_loadouts(coregame.get_coregame_match_id(
                ...
                with richConsole.status("Loading Players...") as status:
                    ...
                    for player in Players:
                        ...
                        peakRankAct = f" (e{playerRank['peakrankep']}a{
                            playerRank['peakrankact']})"
                ...
                with richConsole.status("Loading Players...") as status:
                    ...
                    for player in Players:
                        ...
                        if player["Subject"] == Requests.puuid:
                            ...
                            if OPEN_PROFILES: name, tag = player["Subject"].split("#")
            ...
            if title is not None:
                ...
                table.set_runtime_col_flag('RR', cfg.table.get(
                    "rr") and not cfg.get_feature_flag("aggregate_rank_rr"))
                ...
                if cfg.get_feature_flag("last_played"):
                    if len(already_played_with) > 0:
                        ...
                        for played in already_played_with:
                            print(f"Already played with {played['name']} (last {played['agent']}) {
                                  stats.convert_time(played['time_diff'])} ago. (Total played {played['times']} times)")
                            chatlog(f"Already played with {played['name']} (last {played['agent']}) {
                                    stats.convert_time(played['time_diff'])} ago. (Total played {played['times']} times)")
                already_played_with = []
        if cfg.cooldown == 0:
